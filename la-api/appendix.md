...
| `authorityOfficeCode` | string | **Required.** The sampling office's full code: your authority code followed by the office's own code, e.g. `806HQ` for office `HQ` in authority `806`. Must be an office set up for your authority in UKFSS, otherwise the sample is rejected with `Invalid authorityOfficeCode` |
